#!/usr/bin/env python3
"""残差偏移扩散模型训练脚本 v2 (Residual Shifting Diffusion Training).

本脚本是 train_diffusion_rs.py 的改进版本，修复了以下问题：
1. NaN 检测位置提前到 optimizer.step() 之前
2. DDP eval_max_batches 使用向上取整
3. EMA 更新时动态获取模型引用
4. vis_gain 改为配置项
5. grad_weight_map 添加注释说明
6. 评估函数增加完整采样评估选项
7. 清理冗余代码
8. 调试日志改为可配置
...
def evaluate_single_step(
    ...
    """执行单步预测评估 (Single-step Evaluation)。
    
    注意: 此函数计算的是单步预测的 x0_pred 与 x0 的差异，
    而非完整采样链的最终结果。用于快速评估训练进度。
    如需评估真实去云质量，请使用 evaluate_full_sampling。
    """
    backup = None
    ...
    with torch.no_grad():
        for step_idx, (s1, s2_cloudy, s2_clear, _alpha) in enumerate(iterator, start=1):
            ...
            with torch.amp.autocast(amp_device, enabled=amp_enabled):
                ...
                # 注意: grad_weight_map 为全 1，表示对所有区域同等加权
                # 如需对云边缘区域加权，可使用 alpha 通道计算边缘掩码
                loss_grad = grad_l1_loss(x0_pred, x0, torch.ones_like(x0[:, :1, :, :])) * time_scale
...
def evaluate_full_sampling(
    ...
    """执行完整采样评估 (Full Sampling Evaluation)。
    
    此函数使用完整的 DDIM/DDPM 采样链生成去云结果，
    然后与真实清晰图像进行对比。这是真实去云质量的评估。
    注意: 此函数计算量较大，建议仅在关键 epoch 使用。
    """
    backup = None
    if use_ema and ema is not None:
        backup = apply_ema_weights(model, ema)
    ...
    sampling_steps = sampling_cfg.get("steps", 50)
...
def save_vis_samples(
    ...
    try:
        ...
        # [修复 #3] vis_gain 改为配置项，默认 5.0
        vis_gain = float(vis_cfg.get("vis_gain", 5.0))
        ...
        # 可视化增益：将图像亮度放大以提高可见度 (应对数据本身偏暗的问题)
        ...
        for row, idx in enumerate(indices):
            ...
            for i, step_cnt in enumerate(vis_steps):
                ...
                # [修复 #11] 移除冗余的 pred_out_ratio 计算
                # 由于 pred_np 已经 clamp 过，这个值永远为 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Residual Shifting Diffusion Model (v2)")
    ...
    # [修复 #7] DDP eval_max_batches 使用向上取整
    eval_max_batches = int(eval_cfg.get("max_batches", 0))
    if ddp and eval_max_batches > 0:
        # 使用向上取整确保覆盖所有 batch
        eval_max_batches = max(1, (eval_max_batches + world_size - 1) // world_size)
    ...
    ema = EMA(get_base_model(), decay=cfg["train"].get("ema_decay", 0.999))
    ...
    if is_main and log_path is not None:
        logger = logging.getLogger("train_diffusion_rs_v2")
        logger.setLevel(logging.INFO)
        logger.handlers = []
        file_handler = logging.FileHandler(log_path, encoding="utf-8")
        file_handler.setFormatter(logging.Formatter("%(asctime)s %(message)s", "%Y-%m-%d %H:%M:%S"))
        logger.addHandler(file_handler)
        log_message(f"[Residual Shifting Diffusion v2] 运行时间戳: {run_ts}", logger, console=False, use_tqdm=False)
    ...
    # 配置评估选项
    eval_full_sampling = cfg.get("eval", {}).get("full_sampling", False)
    full_sampling_interval = cfg.get("eval", {}).get("full_sampling_interval", 10)
    full_sampling_max_batches = cfg.get("eval", {}).get("full_sampling_max_batches", 5)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for s1, s2_cloudy, s2_clear, _alpha in iterator:
            ...
            # [修复 #10] NaN/Inf 检测提前到 optimizer.step() 之前
            # 这样可以避免 NaN 污染模型权重和 EMA
            ...
            # [修复 #1] EMA 更新时动态获取模型引用
            ema.update(get_base_model())
        ...
        # [修复 #5] 在指定间隔执行完整采样评估
        full_sampling_metrics = None
        if eval_full_sampling and (epoch + 1) % full_sampling_interval == 0:
            eval_model = get_base_model()
        ...
        if is_main:
            ...
            if full_sampling_metrics is not None:
                log_message(
                    f"Epoch {epoch+1}/{num_epochs} - [Full Sampling] "
                    f"MAE {full_sampling_metrics.get('mae', 0):.4f} "
                    f"PSNR {full_sampling_metrics.get('psnr', 0):.2f} "
                    f"SSIM {full_sampling_metrics.get('ssim', 0):.4f}",
                    logger, console=True, use_tqdm=True
                )
            ...
            save_vis_samples(
                ...
                ema=ema, use_ema=True, debug_log=args.debug,
            )
        ...
        if is_main:
            model_state = get_base_model().state_dict()
            checkpoint = {
                "epoch": epoch,
