#!/usr/bin/env python3
"""Train standard DDPM diffusion with a transformer denoiser."""
...
def evaluate(
    ...
    model.eval()
    metric_keys = base.LOSS_KEYS + base.IMAGE_METRIC_KEYS
    ...
    if ddp:
        vals = [totals[key] for key in metric_keys] + [float(steps), float(sample_count)]
        ...
        res: dict[str, float] = {}
        for i, key in enumerate(base.LOSS_KEYS):
            res[key] = metrics_tensor[i].item() / total_steps
        offset = len(base.LOSS_KEYS)
        for j, key in enumerate(base.IMAGE_METRIC_KEYS):
            res[key] = metrics_tensor[offset + j].item() / total_samples
        return res

    if steps == 0 or sample_count == 0:
        return {key: float("nan") for key in metric_keys}
    out = {key: totals[key] / steps for key in base.LOSS_KEYS}
    out.update({key: totals[key] / sample_count for key in base.IMAGE_METRIC_KEYS})
    return out
    ...
    try:
        ...
        with torch.inference_mode():
            for step_cnt in vis_steps:
                ...
                vis_metrics = base.mean_image_metrics(pred_cpu, x0_cpu, ("psnr", "ssim", "mae"))
